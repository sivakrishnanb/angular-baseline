define(['app', 'model/orders/details', 'utility/messages'], function (app, model, messages) {
    app.controller('CreateOrders', ['$scope', '$bus', '$location', 'ngProgress', '$constants', '$rootScope', '$timeout', '$window', 'notify','highlight','$routeParams',
        function ($scope, $bus, $location, ngProgress, $constants, $rootScope, $timeout, $window, notify,highlight,$routeParams) {
			...
			$scope.getDisplayOrderId = function() {
				return ($scope.model.displayableOrderId && $scope.model.displayableOrderId.length > 20)?false:true;
			}
            ...
            $scope.updateProductBasedCalc = function () {
                ...
                //if(!$scope.model.shipping.liabilityValue) {
            ...
            $scope.validateDate = function (inputDate) {
                ...
                return $scope.displayableOrderDate_errorMessage
            };
            ...
            $scope.createProcessDateChanged = function () {
                ...
                if (!date.match(validDateFormat)) {
                    ...
                    return $scope.processOrder_isDateInvalid 	= true;
                }
            ...
            $scope.createService = function() {
                ...
                $bus.fetch({
                            ...
                            .done(function (success) {
                                if (success.response.success && success.response.success.length) {
                                    $scope.orderCreated = true;
									notify.message(messages.orderCreateSuccess.replace('##',$scope.model.merchantOrderId?$scope.model.merchantOrderId:''),'','succ');
            ...
            $scope.getFullfillableRow = function(param,paramIndex) {
                ...
                if (param.length && typeof(param[3])!='undefined' && param[3]=='0' && $scope.countryOfOriginCtrl.countryCode!=$constants.currentLocation){
            ...
            $scope.getOrderStatus = function(status){
              ...
              $scope.getDuplicateOrderDetails = function(orderHeaderId){
                var params = {
                    id: orderHeaderId || ''
                }
                $scope.isInternationalOrder = false;
            ...
            $scope.init = function () {
                ...
                if($routeParams.clone)
                    $scope.getDuplicateOrderDetails($routeParams.clone);
