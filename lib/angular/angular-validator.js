...
angular.module('angularValidator').directive('angularValidator',
	['$timeout', '$window','$translate', '$filter','$rootScope', function($timeout,$window,$translate, $filter, $rootScope) {
		return {
			restrict: 'A',
			link: function(scope, element, attrs, fn) {
				...
				element.on('submit', function(event) {
					...
					if (scopeForm.$valid) {
                        ...
                        $(DOMForm).find('input,textarea,select').filter('.has-error:first').focus();
                        //$window.scrollTo(0,0);
                    }
				});

				// Setup watches on all form fields 
				$timeout(function(){setupWatches(DOMForm)});
				...
				function updateValidationMessage(element) {
					...
					var defaultRequiredMessage = function() {
						return "<i class='fa fa-times'></i> "+$rootScope.messages('validatorRequired');
					};
					var defaultInvalidMessage = function() {
						return "<i class='fa fa-times'></i> "+$rootScope.messages('validatorInvalid');
					};
					...
					if (scopeElementModel.$dirty || scope[DOMForm.name].submitted) {
				...
				function updateValidationClass(element) {
					...
					if (formField.$dirty || scope[DOMForm.name].submitted) {
						if (formField.$valid) {
